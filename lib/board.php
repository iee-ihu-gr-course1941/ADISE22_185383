...
function show_board($input) {
    ...
    if ($b) {
        ...
        header('Content-type: application/json');
        print json_encode(db_read_board(), JSON_PRETTY_PRINT);
    }
}
...
function add_valid_moves_to_piece(&$board, $b, $x, $y) {
    ...
    if ($board[$x][$y]['piece_color'] == $b) {
        switch ($board[$x][$y]['piece']) {
            case 'P': $number_of_moves += pawn_moves($board, $b, $x, $y);
                break;
            case 'K': $number_of_moves += king_moves($board, $b, $x, $y);
                break;
            case 'Q': $number_of_moves += queen_moves($board, $b, $x, $y);
                break;
            case 'R': $number_of_moves += rook_moves($board, $b, $x, $y);
                break;
            case 'N': $number_of_moves += knight_moves($board, $b, $x, $y);
                break;
            case 'B': $number_of_moves += bishop_moves($board, $b, $x, $y);
                break;
        }
    }
    return($number_of_moves);
...
function db_update_board_after_dealing($cards) {
    global $mysqli;
